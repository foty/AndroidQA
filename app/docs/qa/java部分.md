...
> 面成为该链表的一个新节点。jdk1.8后，引入红黑树，当链表长度大于8时，链表转红黑树；当长度小于6的时，红黑树转链表。
...
put：
> 1、计算key的hash值；2、计算key在数组中的索引(hash&(arr.length-1)；3、根据key的hash值以及当前索引下数组元素
> 判断新元素是放在数组中作为链表表头节点还是链表新节点还是红黑树中的新节点。

get:
> 1、计算key的hash值；2、计算key在数组中的索引(hash&(arr.length-1)；3、key的hash值判断，如果是链表头节点则返回
> 头结点；如果不是，判断表头节点是否属于红黑树。如果是，通过红黑树的查找方式遍历寻找，否则根据比遍历链表找到指定元素。
